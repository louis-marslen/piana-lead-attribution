...
For reference, the master currently contains ~12 filters (proprietaire_prospection IS_UNKNOWN, statut_de_prospection IS_UNKNOWN, siret_2/address/city/zip IS_KNOWN, data___calc__nb_vehicles BETWEEN 5 and 400, ready_tbc IS_EQUAL_TO 1, company_nb_open_leads IS_LESS_THAN 1, num_associated_deals IS_LESS_THAN 1, groupe_verrouille_pour IS_UNKNOWN, admin_date_lead_cloture_la_plus_recente IS_UNKNOWN) — but **always use what the live GET returns**, not this list. If the GET fails, fall back to the reference filters above and flag it in the brief.
